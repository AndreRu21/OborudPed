...
FROM            Oborud INNER JOIN
						 ...
						 where ZayavkaSpis.IDplace = {idplace} and Users.IdUser = {idUserLabel1.Text}", con);
            SqlDataReader rez1 = query1.ExecuteReader();
            while (rez1.Read())
            {
                dgvZayav.Rows.Add(rez1["Fam"], rez1["Nazv"], rez1["Prichina"], rez1["Prinyat"], rez1["DateZayavka"], rez1["SerNomer"], rez1["IdSpis"]);
            }
            con.Close();
        }
...
Where IdPlace = {idplace} and IsSpis = 'true'", con);
        ...
        {
            FormSpisObor frm = new FormSpisObor();
            frm.bsUsers.Filter = this.bsUsers.Filter;
            try
            {
                frm.bsOborud.Filter = "IdObor = " + dgvObor.CurrentRow.Cells[6].Value.ToString();
            }
            catch { MessageBox.Show("Выделите нужное оборудование!", "Внимание!"); return; }
            if(frm.ShowDialog() == DialogResult.OK)
            {
                dgvZayav.Rows.Clear();
                SqlConnection con = new SqlConnection(Properties.Settings.Default.OborudPedConnectionString);
